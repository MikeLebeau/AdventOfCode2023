...
function two(useRealPuzzle: boolean = true): String {
  ...
  const splittedSeedLine = lines[0].split(/\s+/).slice(1);
  for(let i = 0; i < splittedSeedLine.length; i++){
    const seedBase = splittedSeedLine[i];
    const seedCount = Number(splittedSeedLine[i+1]);
    
    for(let j = 0; j < seedCount; j++){
      seeds.push(new Seed(Number(seedBase)+j));
    }
